...
## Быстрый запуск и деплой через `/opt`

Упакуйте проект и запустите интерактивный мастер установки:

```bash
# 1. Собрать архив проекта (создаст scooter-service-bot.tar.gz)
./scripts/pack.sh

# 2. Скопировать install.sh и scooter-service-bot.tar.gz в /opt на целевом сервере
# 3. Запустить интерактивный установщик
cd /opt
sudo bash install.sh
```

Установщик сам проверит зависимости (Docker, Compose, Curl), запросит необходимые данные с подсказками, сформирует защищенный `.env`, соберет контейнеры, проверит `/healthz` и настроит ежедневный бэкап.

## Локальный запуск (ручной)
